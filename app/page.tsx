...
import { siteSectionAppearanceStyle, siteSectionHeroEdgeAttributes, siteSectionSurfaceStyle } from "@/lib/site-section-appearance";
...
export default async function Home() {
  const [listings, events, jobs, homeHeroMedia, homeHeroAppearance] = await Promise.all([
    getPublishedListings(),
    getPublishedEvents(),
    getPublishedJobs(),
    getSiteMedia("home.hero"),
    getSiteSectionAppearance("home.hero"),
  ]);
  ...
  const homeHeroEdgeAttributes = siteSectionHeroEdgeAttributes(homeHeroAppearance);
  ...
  return (
    <PageCanvas appearanceKey="home.page" tone="cream" className="overflow-x-hidden font-sans text-[#242721]">
      <section className={`site-page-hero border-b border-[#242721]/20 bg-[#f4efe5] ${hasHomeHeroEdge ? "isolate overflow-hidden" : ""}`} style={siteSectionAppearanceStyle(homeHeroAppearance)} {...homeHeroEdgeAttributes}>
        <div className="mx-auto grid max-w-[1440px] lg:grid-cols-[1.15fr_0.85fr]">
          <div className="px-5 py-16 sm:px-8 sm:py-24 lg:px-12 lg:py-32">
            ...
            <h1 className="section-appearance-heading-font mt-6 max-w-4xl text-5xl leading-[0.98] tracking-[-0.045em] text-[color:var(--section-heading-color,#22251f)] sm:text-6xl lg:text-7xl xl:text-8xl">The hunter-jumper world, all in one place.</h1>
            <p className="section-appearance-body-font mt-7 max-w-2xl text-lg leading-8 text-[color:var(--section-body-color,#4f514a)] sm:text-xl">Horses, ponies, professionals, horse shows, jobs, and the conversations happening between the rings.</p>
          ...
          <div className="relative min-h-[440px] border-t border-[#242721]/20 bg-[#d9e0de] p-5 sm:p-8 lg:min-h-0 lg:border-l lg:border-t-0 lg:p-12">
            ...
            <div className="relative flex h-full min-h-[400px] flex-col justify-between bg-[#355343] p-6 text-[#f9f4eb] sm:p-8" style={homeHeroSurfaceStyle}>
              ...
              <div className="relative max-w-sm"><p className="section-appearance-heading-font text-4xl leading-none tracking-[-0.04em] text-[color:var(--section-heading-color,#f9f4eb)] sm:text-5xl">Where the good rounds lead.</p><p className="section-appearance-body-font mt-5 max-w-xs text-sm leading-6 text-[color:var(--section-body-color,#e8e3d7)]">A better way to keep up with the listings, people, and showgrounds shaping the season—without the endless group-chat scroll.</p></div>
              <div className="relative grid grid-cols-[1fr_auto] gap-4 border-t border-[#f9f4eb]/25 pt-5"><p className="text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-[color:var(--section-eyebrow-color,#d8bd85)]">From the in gate</p><p className="text-right text-xs text-[color:var(--section-metadata-color,#f9f4eb)]">Hunter · Jumper · Eq · Pony</p></div>
            </div>
          </div>
        </div>
        {homeHeroHasSoftFade ? <span className="site-page-hero-edge-fade pointer-events-none absolute inset-x-0 bottom-0 z-10" aria-hidden="true" /> : null}
      </section>
      ...
      <SectionBackground mediaKey="home.community_background" appearanceKey="home.community" className="border-y border-[#242721]/20 bg-[#dce4e4] px-5 py-16 sm:px-8 sm:py-22 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-[1344px]">
